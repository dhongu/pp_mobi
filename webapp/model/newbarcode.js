sap.ui.define(function() {
	...
	var BarCodeScanner = {
		...
		init: function() {
			...
			this.__keydown_handler = this.bind(this.keydown_handler, this);
			this.__keyup_handler = this.bind(this.keyup_handler, this);
			this.__handler = this.bind(this.handler, this);
		...
		resend_buffered_keys: function() {
			...
			for (var i = 0; i < this.buffered_key_events.length; i++) {
				...
				if ( this.element_is_editable(old_event.target)){
					old_event.target.value = old_event.target.value + old_event.key;
				}
		...
		handler: function(e) {
			...
			this.timeout = setTimeout(this.bind(this.handle_buffered_keys, this), this.max_time_between_keys_in_ms);
			/*
