...
sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	function (Controller, JSONModel, OrderListModel, Button, Dialog, Text, History, formatter,
		Filter, Sorter, MessageToast, MessageBox,
		BarCodeScanner) {
		...
		return Controller.extend("sap.ui.pp.mobi.controller.OrderDetail", {
			...
			doFinishMatched: function (oEvent) {
				...
				oData.Order.HEADER.REPPOINT = '';
				var oRepPoint = oView.byId("RepPoint");
				oRepPoint.clearSelection();
			...
			doSave: function (oPrint) {
				...
				if (oData.Order.HEADER.ZZPKNB == 'X' && oData.Order.HEADER.REPPOINT == '') {
			...
			handleChangeYeld: function (oEvent) {
				...
				if (newValue > oData.Order.HEADER.TOTAL_PLORD_QTY) {
					MessageBox.confirm('Produceti o cantitate mai mare decat cea planificata?',
						function (bResult) {
							if (bResult != 'OK') {
			...
			doCheckBatch: function () {
				...
				for (var comp in oData.Order.COMPONENTS) {
					var REPPOINT = oData.Order.HEADER.REPPOINT.substring(0, 4);
					if (REPPOINT == '' || REPPOINT == oData.Order.COMPONENTS[comp].ACTIVITY) {
			...
			doSetColor: function (IsYeld, EditCom) {
				...
				if (IsYeld) {
					var sId = oFieldGroupView.getId();
					//$('#' + olabelYeld.sId).css("color", "Green");
					$('#' + sId).removeClass("sapScrap");
					$('#' + sId).removeClass("sapScrapComp");
					$('#' + sId).addClass("sapConf");
					...
					$('#' + sId).removeClass("sapConf");
					if (EditCom) {
						$('#' + sId).addClass("sapScrapComp");
						$('#' + sId).removeClass("sapScrap");
					} else {
						$('#' + sId).addClass("sapScrap");
						$('#' + sId).removeClass("sapScrapComp");
					}
			...
			getMaterial: function (vMaterial) {
				var oMaterials = this.getView().getModel("MaterialsCollection").getData();
				oMaterials = oMaterials.ROOT.MATERIALS;
				var material = false;
				for (var i in oMaterials) {
					material = oMaterials[i];
					if (material.MATERIAL.includes(vMaterial)) {
						break;
					}
				}
				return material;
			...
			handleValueRequestCharg: function (oController) {
				...
				this.material = this.getMaterial(value.MATERIAL);
				...
				var oFilterLGORT = new Filter("LGORT", sap.ui.model.FilterOperator.EQ, value.ISS_ST_LOC);
				//var oSorter = new Sorter({path: 'StockCollection>LWEDT', descending: false });
			...
			_handleValueHelpCloseCharg: function (oEvent) {
				...
				if (oSelectedItem) {
					if (!oDataDetail.EnableSelLot && this.material.SEL_LOT != 'X') {
						MessageToast.show("Lotul trebuie scanat");
						return;
					}
					...
					productInput.setValue(valueCharg.CHARG);
					...
					value.SEL_LOC = valueCharg.LGORT;
			...
			onPressSplitCharVal: function (oEvent) {
				var oView = this.getView();
				var oData = oView.getModel().getData();
				var oItem = oEvent.getSource();
				var oContext = oItem.getBindingContext();
				...
				var index = 0;
				for (var i in oData.Order.CHARS) {
					var char_val = oData.Order.CHARS[i];
					if (value.ATNAM == char_val.ATNAM) {
						index = i;
					}
				}

				oData.Order.CHARS.splice(index + 1, 0, NewValue);
			...
			_handleValueHelpConfirmCharVal: function (oEvent) {
				var oSelectedItem = oEvent.getParameter("selectedItem");
				if (oSelectedItem) {
				...
				if (aContexts && aContexts.length) {
					var oContextCharg = aContexts[0];
					var valueChar = oContextCharg.getProperty();
				}
