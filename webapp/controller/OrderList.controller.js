	...
	return Controller.extend("sap.ui.pp.mobi.controller.OrderList", {
		...
		loadOrders: function (oParam) {
			...
			oOwner._loadResource(oDataModel, oParam, function (oRequest) {
				oView.setBusy(false);
				var success = oRequest.getParameter("success");
				var oAllDataModel = oRequest.getSource();
				if (success) {
					...
					var oMaterialsData = {
						"ROOT": {
							"MATERIALS": oAllData.ROOT.MATERIALS
						}
					};
					var oMaterialsModel = new JSONModel(oMaterialsData);
					oOwner.setModel(oMaterialsModel, "MaterialsCollection");
		...
		onChangeProductionLine: function (oEvent) {
			...
			var value = oItem.getProperty('selectedKey'); //oItem.mProperties.selectedKey;
			this.loadOrders({
				'detail': 'line',
				'prod_line': value
			});
		...
		onPressOrder: function (oEvent) {
			...
			oData.Order = oContext.getProperty();
			...
			// merg in SAP si citesc aceasta comanda, la citire in SAP se va face si lock pe materiale
			var oDataModel = new JSONModel();
			var oParam = {
				"detail": "order_lock",
			...
			var oOrderModel = oOwner.getModel("orderlist");

			if (oOrderModel.TestData) {
				...
				oOwner._loadResource(oDataModel, oParam, function (oRequest) {
					...
					if (success) {
						if (oAllDataModel.oData.ROOT.RETURN.TYPE != 'E') {
							...
							var oStockData = {
								"ROOT": {
									"STOCK": oAllDataModel.oData.ROOT.STOCK
								}
							};
							...
							var oMaterialsData = {
								"ROOT": {
									"MATERIALS": oAllDataModel.oData.ROOT.MATERIALS
								}
							};
							var oMaterialsModel = new JSONModel(oMaterialsData);
							oOwner.setModel(oMaterialsModel, "MaterialsCollection");
